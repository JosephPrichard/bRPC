...
message Board struct {
    required turn 1 = 1; // a 1 bit field, it can either be white or black's turn
    required color [64]2 = 2; // a 128 bit field that will be deserialized as a 64 length array
}

message Move struct {
    required row 3 = 1; // a 3 bit field, since this can only contain values 0-7
    required col 3 = 2;
    required disc 1 = 3; // a 1 bit field, this can either be white or black
}

message GameResult struct {
    required winnerId 128 = 1;
    required loserId 128 = 2;
...
message Game struct {
    required id 128 = 4; // we can add fields in any order in the program, the assigned 'ord' value determines the order in serialization
    required whiteId 128 = 1; // storing a uuid in a 128 bit field
    required blackId 128 = 2;
...
service OthelloService {
    operation MakeMove (move Move) -> (err 8); 
    operation GetGame (id 128) -> (err 8 = 1, game Game = 2); // 'ord' is required when more than one tuple field is provided, otherwise it defaults to 1
}
```